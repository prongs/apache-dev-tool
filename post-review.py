...
def get_post_review_dir():
    post_review_path = os.path.join(os.getenv('HOME'), ".post-review")
    if not os.path.exists(post_review_path):
        os.mkdir(post_review_path)
    return post_review_path

def load_jira_to_rbt_map():
    return {}
...
def get_jira():
    ...
    post_review_path = get_post_review_dir()
...
def main():
    ''' main(), shut up, pylint '''
    jira_to_rbt_map = load_jira_to_rbt_map()
    jira = get_jira()
    rbclient = get_rbclient().get_root()
    popt = argparse.ArgumentParser(description='Apache Lens patch review tool')
    popt.add_argument('-b', '--branch', action='store', dest='branch', required=True,
                      help='Tracking branch to create diff against', default="master")
    popt.add_argument('-j', '--jira', action='store', dest='jira', required=True,
    ...
    popt.add_argument('-t', '--testing-done', action='store', dest='testing', required=False,
    ...
    sys.exit(0)
    patch_file = tempfile.gettempdir() + "/" + opt.jira + "." + opt.version + ".patch"
    if opt.reviewboard:
        ts = time.time()
    ...
    os.system("git merge " + opt.branch)
    os.system("git mergetool")
    os.system('git commit -am "merge with ' + opt.branch + '"')
    opt.jira = opt.jira.upper()
    if not opt.reviewboard:
        jira_to_rbt_map.get(opt.jira)
    rbclient.get_review_request(review_request_id=33943).get_diffs()
    rb_command = "post-review --publish --tracking-branch " + opt.branch + " --target-groups=lens --bugs-closed=" + opt.jira
